...
// Function to find the miniimum of 2 numbers
...
struct node* CreateNode(int key, int depth, int MnDt, bool isRoot)
{	
...
struct node* BuildTree(int start, int end, int depth, bool isRoot)
{
		...
		// Terminiating condition for this recursive function
    if(start==end)
    {
        return NULL;
    }
...
void Rotate(struct node * ptr)
{
    ...
    if(temp->leftChild!=NULL)
    {
        temp->MnDt = mini(temp->MnDt, temp->leftChild->MnDt);
    }
    if(temp->rightChild!=NULL)
    {
        temp->MnDt = mini(temp->MnDt,temp->rightChild->MnDt);
    }
}
...
struct node* refParent(struct node* ptr,bool flag)
{
    ...
    while(1)
    {
        if(!flag)
        {
            if(temp->rightChild!=NULL && temp->rightChild->MnDt < ptr->depth) 
            {
                temp = temp->rightChild;
            }
            else if(temp->leftChild!=NULL && temp->leftChild->MnDt < ptr->depth)
            {      
                temp = temp->leftChild;
            }
            else
        ...
        {
            if(temp->leftChild && temp->leftChild->MnDt < ptr->depth)
            {
                temp = temp->leftChild;
            }
            else if(temp->rightChild && temp->rightChild->MnDt < ptr->depth)
            {
                temp = temp->rightChild;
            }
            else
...
void SwitchPath(struct node * ptr)
{
    ...
    if(ptr->leftChild!=NULL)
    {
        if(ptr->leftChild->MnDt > ptr->depth)
        {
    ...
    if(ptr->rightChild!=NULL)
    {
        if(ptr->rightChild->MnDt > ptr->depth)
        {
